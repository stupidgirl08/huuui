...
from ..SQL.global_collection import (
    del_keyword_collectionlist,
    get_item_collectionlist,
)
from ..SQL.globals import addgvar, delgvar, gvarstatus
    ...
    try:
        ...
        for option in config.dc_options:
            if option.ip_address == client.session.server_address:
                ...
                client.session.save()
                break
        bot_details = await client.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        # await catub.start(bot_token=Config.TG_BOT_USERNAME)
        client.me = await client.get_me()
        client.uid = client.tgbot.uid = utils.get_peer_id(client.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(client.me)
...
async def startupmessage():
    ...
    try:
        if msg_details:
            await client.check_testcases()
            message = await client.get_messages(msg_details[0], ids=msg_details[1])
            text = message.text + "\n\n**Ok Bot is Back and Alive.**"
            await client.edit_message(msg_details[0], msg_details[1], text)
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await client.get_entity(BOTLOG_CHATID)
        ...
        _, groupid = await create_supergroup(
            "CatUserbot BotLog Group", client, Config.TG_BOT_USERNAME, descript
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await client.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info(
