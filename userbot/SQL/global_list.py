...
class Globallist(BASE):
    ...
    def __repr__(self):
        return "<Retro global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, Globallist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


Globallist.__table__.create(checkfirst=True)

GLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with GLOBALLIST_INSERTION_LOCK:
        broadcast_group = Globallist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with GLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(Globallist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with GLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(Globallist.keywoard)
            .filter(Globallist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(Globallist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(Globallist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(Globallist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(Globallist).all()
        for x in all_groups:
